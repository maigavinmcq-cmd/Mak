...
def main() -> None:
    ...
    for text in ["日志工作台", "错误聚合", "PID", "task_id", "关键词"]:
        assert text in dialog_init or text in apply_filters, f"log workbench missing {text}"
    assert "hide_diagnostics_check" in dialog_init
    assert "PERF" in apply_filters and "诊断" in dialog_init
