...
class TikTokDownloaderApp:
    def __init__(self, root):
        ...
        self.log_text = scrolledtext.ScrolledText(root, height=1, state='disabled', bg="#f0f0f0")
        ...
        self.count_label = tk.Label(root, text="已完成：0 / 0", fg="green")
        ...
        self.count_label = tk.Label(root, text="准备下载视频数: 0 / 已完成: 0")
    ...
    def resolve_short_link(self, url):
        ...
        try:
            ...
            if 'tiktok.com/@' in final_url and '/video/' in final_url:
                ...
                print(f"⚠️ 非标准跳转链接：{final_url}")
                return url
        except Exception as e:
            print(f"⚠️ 短链解析失败：{url} 错误：{e}")
            return url
    ...
    def download_videos(self):
        ...
        resolved_links = []
        for link in links:
            if "tiktok.com/t/" in link:
                try:
                    resolved = self.resolve_short_link(link)
        ...
        # ✅ 更新链接统计状态
        self.link_stats_label.config(
            text=f"共输入：{len(links)} 条，去重后：{len(unique_links)} 条"
        )
        ...
        for i, url in enumerate(unique_links, 1):
            try:
                ...
                self.update_count_label()
                self.log_to_ui(f"[{i}] 下载完成：{url}")
            except Exception as e:
                self.log_to_ui(f"[{i}] 下载失败：{url} 错误：{e}")
    ...
    def download_single_video(self, url, save_path):
        ...
        filename = f"tiktok_{timestamp}.%(ext)s"
