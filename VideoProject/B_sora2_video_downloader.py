...
def find_system_browser_channel():
    candidates = [
        (
            "msedge",
            [
...
def safe_filename_from_url(url: str) -> str:
    ...
    if not name:
        ...
        parts = [seg for seg in decoded_path.split("/") if seg]
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            print(
                ...
                f"已存在 {existing_size} 字节）：{url} -> {file_path}"
            )
...
class VideoDownloaderApp(Tk):
    def __init__(self):
        ...
        self.archive_name_var = StringVar(value="")
    ...
    def _get_effective_download_path(self) -> str:
        archive_name = self._sanitize_archive_name(self.archive_name_var.get())
        if archive_name:
            return os.path.join(self.download_path, archive_name)
    ...
    def create_widgets(self):
        ...
        archive_frame = Frame(top_frame)
        archive_frame.pack(fill="x", pady=3)

        Label(archive_frame, text="归档文件夹名：").pack(side="left")
        self.archive_name_entry = Text(archive_frame, height=1, width=24)
        self.archive_name_entry.pack(side="left", padx=(6, 10))
        self.archive_name_entry.bind("<KeyRelease>", lambda e: self.archive_name_var.set(
            self.archive_name_entry.get("1.0", "end-1c").strip()
        ))
        Label(
            archive_frame,
            text="留空则直接下载到保存位置；填写后将自动归档到该子文件夹",
            fg="#666",
            anchor="w",
            justify="left",
        ...
        Label(headless_frame, text='' """f"重定向前缀：{REDIRECT_PREFIX_DEFAULT}").pack(side="left", padx=(12, 0)""")
    ...
    def headless_activate_redirects(self, redirect_urls: list[str], concurrency: int = 3):
        ...
        """
        bundled_path, has_bundled_browser, why = ensure_playwright_browsers_path()
        system_channel, system_browser_path = find_system_browser_channel()
        self.log(f"Playwright browsers path = {bundled_path} ({why})")
        if system_channel and system_browser_path:
            self.log(f"检测到系统浏览器：{system_channel} | {system_browser_path}")
        if not has_bundled_browser and not system_channel:
            self.log("❌ Headless 无法启动：未找到内置 Chromium，也未检测到系统 Edge/Chrome。")
                ...
                async def activate_one(url_item: str, idx: int):
                    async with sem:
                        ...
                        try:
                            ...
                            self.log(f"[Headless激活 {idx}/{len(redirect_urls)}] {url_item}")
                            ...
                            self.log(f"  -> FINAL: {final_url}")
                            await asyncio.sleep(delay_f)
                            return True
                        except Exception as e:
                            self.log(f"❌ [Headless FAIL] {url_item}  ERROR: {e}")
    ...
    def start_download(self):
        ...
        effective_outdir = self._get_effective_download_path()
        self.log(f"本次实际保存目录：{effective_outdir}")
    ...
    def download_one(self, url: str, index: int, already_real: bool = False):
        ...
        outdir = self._get_effective_download_path()

        if is_tiktok_url(real_url):
            self.log(f"[开始下载 TikTok 第 {index:03d} 条] {real_url}")
            err = download_tiktok(
                real_url,
                outdir,
            ...
            self.log(f"[开始下载 第 {index:03d} 条] {real_url}")
        ...
        if err:
            self.failed_downloads += 1
            self.log(f"[下载失败 第 {index:03d} 条] {real_url} - {err}")
        else:
            self.successful_downloads += 1
            self.log(f"[下载成功 第 {index:03d} 条] {real_url}")
        return err
