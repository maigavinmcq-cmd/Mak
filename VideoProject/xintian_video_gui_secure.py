# -*- coding: utf-8 -*-
"""
图生视频（AUTO/双API·多任务并发·任务记忆·Mission批量·导入导出·筛选·归档下载·加密配置·带日志）
✅ 新增：计费账本（每日花销统计，按 0.4 元/请求）
✅ 新增：超时/中断任务追踪（XINTIAN 可追踪 remote_id，随时补查是否返回下载链接）
✅ 新增：一键补查“失踪任务”（筛选范围/选中任务）+ 可选自动定时补查

✅ 本次按你所有需求修复/增强：
1) 修复：XINTIAN拿到remote_id后不应立刻变 Pending(Check)，否则 UI 进度永远不更新
2) 修复：进度更新不再只认 Running（Running/Pending 都可更新）
3) 新增：暂停(Stop) + 继续(Resume)按钮
   - XINTIAN 有 remote_id：Resume=继续轮询同一个 remote_id（不重新POST、不扣费）
   - APIYI 无 remote_id：Resume=重新执行（会发请求扣费）
4) 新增：程序重启恢复向导
   - 启动后检测历史任务：remote_id任务是否仍在跑/是否已出链接、Queued/Failed等是否继续执行
   - 弹窗征求你同意：仅继续remote_id / remote_id+启动队列 / 先不动
   - 不允许未经同意自动开跑
5) AutoRetry “上锁”：未经过恢复向导确认前，不会自动重试
6) 退出软件≠任务报废：XINTIAN remote_id 可补查/可继续轮询找回链接
"""
...
# =========================
# ✅ Queue Gate（达到阈值就暂停队列，不再启动新任务）
# =========================
QUEUE_GATE_ENABLED_DEFAULT = False
QUEUE_GATE_REMOTE_ID_LIMIT_DEFAULT = 0   # 0=不启用该指标
QUEUE_GATE_LINK_LIMIT_DEFAULT = 0        # 0=不启用该指标
QUEUE_GATE_POPUP_ONCE = True             # 触发后只弹一次

# =========================
# ✅ Auto Retry (24h)
# =========================
AUTO_RETRY_ENABLED = True
AUTO_RETRY_TICK_SEC = 20          # 每 20 秒扫一遍
AUTO_RETRY_MAX_RETRIES = 50       # 单任务最多重试次数（建议 20~50）
AUTO_RETRY_BASE_DELAY_SEC = 30    # 第一次失败后延迟 30 秒
AUTO_RETRY_MAX_DELAY_SEC = 300   # 退避上限 5 分钟
AUTO_RETRY_INCLUDE_DONE_NO_URL = True

ENABLE_MACHINE_BINDING = False
ALLOWLIST_FILE = "machine.allow"

# ✅ 计费（按你要求：请求成功发出=0.4元；超时中断也算=0.4元）
COST_PER_REQUEST = 0.4
BILLING_LEDGER_FILE = Path(LOG_DIR) / "billing_ledger.jsonl"  # JSONL 事件账本（防崩溃/可追溯）
...
def run_apiyi_sse(cfg: ApiyiConfig, on_text, on_done, on_error, stop_flag: threading.Event):
    """
    ⚠️ APIYI /chat/completions SSE 天然缺少可追踪 remote_id
    - 断线/超时后很可能无法补查（除非服务商另有 status API）
    """
    try:
...
def run_xintian_upload_and_poll(cfg: XintianConfig, on_text, on_remote_id, on_done, on_error, stop_flag: threading.Event):
    """
    ✅ XINTIAN：POST 创建后会返回 id/video_id/task_id，可追踪
    - 我们会在解析到 remote_id 的瞬间调用 on_remote_id() 落盘
    - 后续即使 timeout/断线，你也能用 remote_id 再查询链接
    """
    try:
...
def run_xintian_poll_existing(cfg: XintianConfig, video_id: str, on_text, on_done, on_error, stop_flag: threading.Event):
    """
    ✅ 仅轮询已有 remote_id（用于 Resume/启动恢复/继续等待）
    - 不会重新 POST
    - 不扣费（扣费逻辑应只在创建请求时记）
    """
    try:
        ...
        on_text(f"📡 Status URL: {status_url}\n")
        on_text("🔄 开始轮询进度...\n")
        ...
        while True:
            ...
            try:
                ...
                on_error(f"timeout during polling: {e}")
                return
            except Exception as e:
                on_error(f"polling network error: {e}")
                return
            ...
            if gr.status_code != 200:
                ...
                on_error(f"poll http {gr.status_code}")
                return
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        BILLING_LEDGER_FILE.parent.mkdir(exist_ok=True)
        ...
        # ✅ 自动补查失踪链接（可选）
        self.auto_refresh_lost_var = tk.BooleanVar(value=False)
        self.auto_refresh_interval_var = tk.IntVar(value=60)
        self._auto_refresh_timer = None
        # =========================
        # ✅ Queue Gate runtime state
        # =========================
        self.gate_enabled_var = tk.BooleanVar(value=QUEUE_GATE_ENABLED_DEFAULT)
        self.gate_remote_limit_var = tk.IntVar(value=QUEUE_GATE_REMOTE_ID_LIMIT_DEFAULT)
        self.gate_link_limit_var = tk.IntVar(value=QUEUE_GATE_LINK_LIMIT_DEFAULT)

        self.queue_frozen_by_gate = False
        self._gate_popup_shown = False

        # ✅ 恢复向导确认：未确认前不允许 AutoRetry/自动跑
        ...
        guide_text = (
            ...
            "⑥ ✅ 新增：按 0.4 元/请求的【计费账本】，今日花销可见；XINTIAN 超时/退出可随时补查链接/继续轮询\n"
            "⑦ ✅ 新增：Stop(暂停) + Resume(继续)；程序重启后会弹窗询问是否恢复继续\n"
        )
        ...
        tk.Button(ctrl, text="停止选中任务(暂停)", command=self.stop_selected_task, height=2, width=16).pack(side="left", padx=6)
        tk.Button(ctrl, text="继续选中任务(恢复)", command=self.resume_selected_task, height=2, width=18).pack(side="left", padx=6)
        ...
        tk.Button(ctrl, text="📊 今日花销", command=self.show_today_billing, height=2, width=12).pack(side="left", padx=6)
        ...
        tk.Button(ctrl, text="▶ 继续队列(解除暂停)", command=self.resume_queue_from_gate,
                  height=2, width=18).pack(side="left", padx=6)

        tk.Button(ctrl, text="🔍 补查失踪链接(筛选)", command=lambda: self.refresh_pending_links(scope="filtered"), height=2, width=18)\
            .pack(side="left", padx=6)
        gate_box = tk.Frame(ctrl)
        gate_box.pack(side="left", padx=8)

        tk.Checkbutton(gate_box, text="Gate阈值暂停", variable=self.gate_enabled_var,
                       command=self.on_gate_setting_change).pack(side="left")

        tk.Label(gate_box, text="RemoteID≥").pack(side="left", padx=(8, 2))
        tk.Spinbox(gate_box, from_=0, to=999999, width=6, textvariable=self.gate_remote_limit_var,
                   command=self.on_gate_setting_change).pack(side="left")

        tk.Label(gate_box, text="Link≥").pack(side="left", padx=(8, 2))
        tk.Spinbox(gate_box, from_=0, to=999999, width=6, textvariable=self.gate_link_limit_var,
                   command=self.on_gate_setting_change).pack(side="left")

        # 自动补查控件
        auto_box = tk.Frame(ctrl)
        auto_box.pack(side="left", padx=8)
        tk.Checkbutton(auto_box, text="自动补查", variable=self.auto_refresh_lost_var, command=self.toggle_auto_refresh).pack(side="left")
        tk.Label(auto_box, text="间隔(s)").pack(side="left", padx=(6, 2))
        tk.Spinbox(auto_box, from_=20, to=600, width=5, textvariable=self.auto_refresh_interval_var).pack(side="left")
        ...
        status_box = ttk.Combobox(filter_bar, textvariable=self.filter_status_var,
                                  values=["全部", "Queued", "Running", "Success", "Failed", "Stopped", "Done(No URL)", "Pending(Check)"],
        ...
        cols = ("task_id", "provider", "created_at", "group", "note", "status", "progress", "model", "image", "remote_id", "video_url")
        ...
        headings = [
            ...
            ("model", "模型", 190),
            ("image", "图片文件", 140),
            ("remote_id", "Remote ID(可追踪)", 180),
            ("video_url", "视频链接（如有）", 320),
        ]
        ...
        self.tree.tag_configure("pending", background="#e9f1ff")
        ...
        self.task_menu.add_command(label="停止任务(暂停)", command=self.stop_selected_task)
        self.task_menu.add_command(label="继续任务(恢复)", command=self.resume_selected_task)
        self.task_menu.add_separator()
        self.task_menu.add_command(label="打开链接", command=self.open_selected_video)
        self.task_menu.add_command(label="🔍 补查此任务链接", command=lambda: self.refresh_pending_links(scope="selected"))
        ...
        self._update_status_with_billing()

        # ✅ 启动恢复向导（不自动跑，先征求你同意）
        self.root.after(250, self.startup_recovery_flow)

    # =========================
    # Billing ledger (JSONL)
    # =========================
    def billing_append(self, payload: dict):
    ...
    def billing_today_stats(self) -> dict:
        stats = {
            "date": today_str(),
            "total_amount": 0.0,
            "request_count": 0,
    ...
    def show_today_billing(self):
        s = self.billing_today_stats()
        lines = []
        lines.append(f"日期：{s['date']}")
        lines.append(f"今日请求数（按 request_sent 计）：{s['request_count']} 条")
        lines.append(f"今日花销（按 {COST_PER_REQUEST:.2f} 元/请求）：¥{s['total_amount']:.2f}")
        if s["by_provider"]:
            lines.append("\n按 Provider：")
            for k, v in sorted(s["by_provider"].items(), key=lambda x: (-x[1], x[0])):
                lines.append(f"  - {k}: {v} 条")
        lines.append(f"\n账本文件：{BILLING_LEDGER_FILE.resolve()}")
        messagebox.showinfo("📊 今日花销", "\n".join(lines))

    def _update_status_with_billing(self):
        s = self.billing_today_stats()
        cur = self.status_var.get().replace("状态：", "", 1).strip()
        extra = f" | 今日请求 {s['request_count']} | 今日花销 ¥{s['total_amount']:.2f}"
        base = cur.split(" | 今日请求 ")[0].strip()
        self.status_var.set(f"状态：{base}{extra}")
        self.root.after(3000, self._update_status_with_billing)

    # =========================
    # Recovery wizard (startup)
    # =========================
    def startup_recovery_flow(self):
        """
        程序启动后的恢复向导：
        - 统计：remote_id 可追踪任务 / queued / failed/done 等
        - 弹窗让你选择恢复策略
        - 未确认前：禁止 AutoRetry 自动跑
        """
        # 统计
        remote_tasks = []
        queued_tasks = []
        ...
        for t in self.tasks.values():
            ...
            if rid and (t.provider in ("xintian", "auto")) and t.status in ("Pending(Check)", "Done(No URL)", "Failed", "Stopped"):
                remote_tasks.append(t)
            if t.status == "Queued":
                queued_tasks.append(t)
            if t.status in ("Failed", "Done(No URL)"):
                retry_tasks.append(t)

        if not remote_tasks and not queued_tasks and not retry_tasks:
            self.recovery_confirmed = True
            if AUTO_RETRY_ENABLED:
                self.root.after(2000, self._auto_retry_tick)
            return

        # 预补查：不扣费、不重新POST（但需要你已加载 XINTIAN Key 才能查）
        def prefetch_then_prompt():
            lines = []
            if remote_tasks:
                lines.append(f"可继续/可补查的 Remote ID 任务：{len(remote_tasks)} 条（不重新扣费）")
            if queued_tasks:
                lines.append(f"待启动 Queued 任务：{len(queued_tasks)} 条（会发请求并扣费）")
            if retry_tasks:
                lines.append(f"可重试 Failed/Done(No URL)：{len(retry_tasks)} 条（会发请求并扣费）")

            msg = "检测到上次未完成/未确认任务：\n\n" + "\n".join(lines) + "\n\n请选择恢复方式："

            win = tk.Toplevel(self.root)
            win.title("恢复任务（上次退出未完成）")
            win.geometry("560x290")
            win.resizable(False, False)

            ttk.Label(win, text=msg, justify="left", wraplength=520).pack(padx=16, pady=16, anchor="w")
            ...
            def do_remote_only():
                ...
                self.recovery_confirmed = True
                if remote_tasks:
                    self.resume_remote_tasks(remote_tasks)
                if AUTO_RETRY_ENABLED:
                    self.root.after(2000, self._auto_retry_tick)

            def do_remote_and_queued():
                win.destroy()
                self.recovery_confirmed = True
                if remote_tasks:
                    self.resume_remote_tasks(remote_tasks)
            ...
            def do_nothing():
                win.destroy()
                self.recovery_confirmed = True
                # 不自动跑；AutoRetry 也不启动（避免你不同意就重跑扣费）
                # 如果你希望“确认后也自动重试”，可在这里启用：
                # if AUTO_RETRY_ENABLED: self.root.after(2000, self._auto_retry_tick)

            ttk.Button(btns, text="✅ 仅继续 Remote ID（不扣费）", command=do_remote_only).pack(side="left")
            ttk.Button(btns, text="🚀 Remote ID + 启动 Queued（扣费）", command=do_remote_and_queued).pack(side="left", padx=10)
            ttk.Button(btns, text="⏸️ 先不动（我手动恢复）", command=do_nothing).pack(side="right")

        # 预补查链接（silent）
        if remote_tasks and self.xintian_key:
            try:
                self.refresh_pending_links(scope="filtered", silent=True)
            except Exception:
                pass
            self.root.after(350, prefetch_then_prompt)
        else:
            self.root.after(0, prefetch_then_prompt)

    # =========================
    # Auto refresh lost links (optional)
    # =========================
    def toggle_auto_refresh(self):
        if self.auto_refresh_lost_var.get():
            self.log_global(f"🔁 已开启自动补查：每 {int(self.auto_refresh_interval_var.get())} 秒（仅补查 XINTIAN 可追踪任务）\n")
            self._schedule_auto_refresh()
        else:
            self.log_global("⏹️ 已关闭自动补查\n")
    ...
    def _schedule_auto_refresh(self):
        if not self.auto_refresh_lost_var.get():
            return
        interval = max(20, int(self.auto_refresh_interval_var.get() or 60))
        if self._auto_refresh_timer is not None:
            try:
                self.root.after_cancel(self._auto_refresh_timer)
            except Exception:
                pass
        self._auto_refresh_timer = self.root.after(interval * 1000, self._auto_refresh_tick)
    ...
    def _auto_retry_tick(self):
        if self.queue_frozen_by_gate or self.maybe_trigger_gate(reason="AutoRetry tick"):
            # Gate冻结：不做自动重试启动
            self.root.after(int(AUTO_RETRY_TICK_SEC * 1000), self._auto_retry_tick)
            return

        """后台自动把到点的失败任务重新入队并开跑。"""
        # ✅ 未经过恢复向导确认，不允许自动重试（避免你不同意就扣费）
        if not getattr(self, "recovery_confirmed", False):
            self.root.after(int(AUTO_RETRY_TICK_SEC * 1000), self._auto_retry_tick)
            return
        ...
        try:
            ...
            for t in self.tasks.values():
                ...
                retryable = (t.status == "Failed") or (AUTO_RETRY_INCLUDE_DONE_NO_URL and t.status == "Done(No URL)")
                if not retryable:
                    continue

                if t.attempts >= AUTO_RETRY_MAX_RETRIES:
                    continue

                dt = self._parse_dt(t.next_retry_at)
                if dt and dt > now:
                    continue
            ...
            if started:
                self.log_global(f"\n♻️ AutoRetry：已自动重试 {started} 个任务\n")

        except Exception as e:
            self.log_global(f"\n⚠️ AutoRetry tick 异常：{e}\n")
        ...
        try:
            ...
            for d in items:
                try:
                    ...
                    # ✅ 恢复策略：Running 不要盲目变 Queued
                    # - 有 remote_id：说明 server 可能还在跑 => Pending(Check)（可补查/可继续轮询）
                    # - 无 remote_id：不可追踪 => Queued（等你决定是否重跑）
                    if t.status == "Running":
    ...
    def gate_counts(self) -> tuple[int, int]:
        """返回 (remote_id_count, success_link_count)，默认去重统计。"""
        remote_set = set()
        link_set = set()
        for t in self.tasks.values():
            rid = (t.remote_id or "").strip()
    ...
    def gate_reached(self) -> bool:
        if not bool(self.gate_enabled_var.get()):
            return False
        r, l = self.gate_counts()
        r_lim = int(self.gate_remote_limit_var.get() or 0)
        l_lim = int(self.gate_link_limit_var.get() or 0)

        cond_r = (r_lim > 0 and r >= r_lim)
        cond_l = (l_lim > 0 and l >= l_lim)
        return cond_r or cond_l

    def maybe_trigger_gate(self, reason: str = "") -> bool:
        """
        如果达到阈值：冻结队列（不启动新任务），可选弹窗一次。
        返回 True 表示 gate 已触发/仍在冻结中。
        """
        if self.queue_frozen_by_gate:
            return True

        if self.gate_reached():
            self.queue_frozen_by_gate = True
            r, l = self.gate_counts()
            msg = f"🧯 Gate触发：已达到阈值，队列已暂停（不再启动新任务）\nRemoteID={r} | Link={l}\n"
            if reason:
                msg += f"原因：{reason}\n"
            self.log_global("\n" + msg + "\n")
            self.set_status("Gate触发：队列已暂停")

            if QUEUE_GATE_POPUP_ONCE and not self._gate_popup_shown:
                self._gate_popup_shown = True
                try:
                    messagebox.showinfo("Gate触发：队列已暂停",
                                        f"达到阈值，队列已暂停。\n\nRemoteID={r}\nLink={l}\n\n你可点击“继续队列(解除暂停)”继续。")
                except Exception:
                    pass
            return True
    ...
    def resume_queue_from_gate(self):
        """手动解除 Gate 冻结，并启动所有 Queued。"""
        self.queue_frozen_by_gate = False
        self._gate_popup_shown = False
        self.set_status("已解除队列暂停")
        self.log_global("\n▶ 已解除 Gate 暂停：允许继续启动新任务\n\n")
        self.start_all_queued()

    def on_gate_setting_change(self):
        """修改 gate 设置后，立即评估一次。"""
        if not self.gate_enabled_var.get():
            # 关闭 gate：自动解除冻结
            if self.queue_frozen_by_gate:
                self.queue_frozen_by_gate = False
                self._gate_popup_shown = False
                self.log_global("\n✅ 已关闭 Gate：解除队列暂停\n\n")
                self.set_status("已关闭 Gate")
            return

        # 开启 gate：检查是否应该立刻冻结
        self.maybe_trigger_gate(reason="设置变更触发检查")
    ...
    def _run_task_async(self, task: TaskItem):
        # ✅ Gate：若已冻结或达到阈值，则不再启动新任务（保持Queued）
        if self.queue_frozen_by_gate or self.maybe_trigger_gate(reason="准备启动新任务"):
            task.status = "Queued"
            task.progress = 0.0
            self.mark_dirty()
            self.request_refresh()
        ...
        def run_one():
            ...
            task.status = "Running"
            task.progress = float(task.progress or 0.0)  # 保留旧进度也可
            task.video_url = None
            task.lost_reason = ""
            task.last_check_at = ""
            ...
            self.log_task(task.task_id, f"🔁 Attempt: {task.run_attempt}\n")

            def on_text(t: str):
                self.log_task(task.task_id, t)
                p = parse_progress_percent(t)
                # ✅ 修复：不再只认 Running（Pending(Check) 也允许更新）
                if p is not None and task.status in ("Running", "Pending(Check)"):
                    task.progress = p
                    self.mark_dirty()
                    self.request_refresh()
            ...
            def on_remote_id(rid: str):
                task.remote_id = rid
                # ✅ 修复：remote_id 不代表任务中断，不要改 Pending(Check)
                task.last_check_at = now_str()
                self.mark_dirty()
                self.request_refresh()
                self.log_task(task.task_id, f"🧷 已记录 Remote ID：{rid}\n")
                self.billing_append({
                    "task_id": task.task_id,
                    "provider": "xintian",
                    "event": "remote_id_obtained",
                    "amount": 0.0,
                    "remote_id": rid,
                    "model": task.model,
                ...
                # ✅ remote_id 增加可能触发 Gate
                self.maybe_trigger_gate(reason="remote_id 达到阈值")
            ...
            def on_done(video_url: str | None):
                ...
                # ✅ 成功链接增加可能触发 Gate
                self.maybe_trigger_gate(reason="video_url 达到阈值")
            ...
            def friendly_provider_error(raw: str) -> str:
                ...
                if "timeout" in low:
                    return "请求超时：网络波动或服务端慢。建议稍后重试；若为 XINTIAN 且已有 Remote ID，可用“补查失踪链接/继续任务”找回。"
                return raw

            def on_error(err: str):
                is_timeout = self._is_timeout_error(err)
                if is_timeout:
                    task.lost_reason = "timeout"
                    # XINTIAN：有 remote_id 则标记 Pending(Check)，允许后续补查/继续
                    if task.remote_id and chosen_provider == "xintian":
                        task.status = "Pending(Check)"
                        self.log_task(task.task_id, f"\n🟦 超时中断：已保留 Remote ID，可后续补查/继续轮询（不算报废）\n")
                    else:
                        task.status = "Failed"
                        task.last_error = friendly_provider_error(err)
                        self.log_task(task.task_id, f"\n❌ 超时中断：{task.last_error}\n")
                else:
                    task.status = "Failed"
                    task.lost_reason = "error"
                    task.last_error = friendly_provider_error(err)

                    # ✅ 计算下次重试时间（指数退避）
                    delay = AUTO_RETRY_BASE_DELAY_SEC * (2 ** max(0, task.attempts - 1))
                    delay = min(delay, AUTO_RETRY_MAX_DELAY_SEC)
                    next_dt = datetime.datetime.now() + datetime.timedelta(seconds=delay)
                    task.next_retry_at = next_dt.strftime("%Y-%m-%d %H:%M:%S")

                    self.log_task(task.task_id, f"\n❌ 出错：{task.last_error}\n")
                    self.log_task(task.task_id, f"♻️ 下次自动重试时间：{task.next_retry_at}\n")
            ...
            self.log_task(task.task_id, f"💸 已记账：request_sent 扣费 ¥{COST_PER_REQUEST:.2f}\n")
            ...
            if chosen_provider == "apiyi":
                ...
                run_xintian_upload_and_poll(cfg, on_text, on_remote_id, on_done, on_error, task.stop_event or threading.Event())
    ...
    def resolve_auto_provider(self, task: TaskItem) -> tuple[str, str, str]:
        ...
        if model_is_apiyi and apiyi_ok and not xintian_ok:
    ...
    def start_all_queued(self):
        if self.queue_frozen_by_gate or self.maybe_trigger_gate(reason="start_all_queued"):
            self.log_global("\n🧯 Gate冻结中：start_all_queued 被拦截（不会启动新任务）\n\n")
            self.set_status("Gate冻结中：队列未启动")
            return
    ...
    def stop_selected_task(self):
        ...
        t.stop_event.set()
        t.status = "Stopped"
        t.lost_reason = "user_stop"
        self.log_task(sel, "\n🟨 已请求暂停该任务（本地停止等待/轮询）...\n")
        self.mark_dirty()
        self.request_refresh()
        self.set_status(f"已暂停 {sel}")
    ...
    def stop_all_tasks(self):
        ...
        self.mark_dirty()
        self.request_refresh()
        self.set_status("已暂停全部任务")

    def resume_selected_task(self):
        """
        ✅ Continue/Resume 按钮逻辑：
        - XINTIAN + remote_id：继续轮询同 remote_id（不重新POST、不扣费）
        - 否则：重新执行（会发请求扣费）
        """
        sel = self.get_selected_task_id()
        if not sel:
        ...
        # 1) 有 remote_id => 继续轮询
        if rid and (t.provider in ("xintian", "auto")):
            if not self.xintian_key:
                messagebox.showwarning("提示", "未加载 XINTIAN Key，无法继续轮询。")
                return
            ...
            def runner():
                def on_text(msg: str):
                    self.log_task(t.task_id, msg)
                    p = parse_progress_percent(msg)
                    if p is not None:
                        t.progress = p
                        self.mark_dirty()
                        self.request_refresh()
                ...
                def on_done(video_url: str | None):
                    ...
                    if t.stop_event and t.stop_event.is_set():
                        t.status = "Stopped"
                    else:
                        t.status = "Success" if video_url else "Done(No URL)"
                    if t.status in ("Success", "Done(No URL)"):
                        t.progress = max(t.progress, 100.0)
                    t.last_check_at = now_str()
                    self.mark_dirty()
                    self.request_refresh()
                ...
                def on_error(err: str):
                    # 轮询失败：仍可补查（不报废）
                    t.status = "Pending(Check)"
                    t.last_error = str(err)[:500]
                    t.last_check_at = now_str()
                    self.log_task(t.task_id, f"\n⚠️ 继续轮询中断：{err}\n")
                    self.mark_dirty()
                    self.request_refresh()

                cfg = XintianConfig(
                    api_key=self.xintian_key,
                    base_url=XINTIAN_DEFAULT_BASE,
                    model=t.model,
                    prompt=t.prompt,
                    image_path=t.image_path,
                    poll_interval_sec=3,
                )
                run_xintian_poll_existing(cfg, rid, on_text, on_done, on_error, t.stop_event or threading.Event())

            if not self.executor:
                self.apply_concurrency()
            t.future = self.executor.submit(runner)
            self.set_status(f"已继续轮询：{sel}（不扣费）")
            return
        ...
        if t.status not in ("Queued", "Failed", "Stopped", "Done(No URL)"):
            if not messagebox.askyesno("确认", f"当前状态为 {t.status}，仍要重新执行一次吗？\n（会扣费）"):
                return

        if not t.image_path or not Path(t.image_path).exists():
            messagebox.showerror("错误", "图片路径不存在（可能被移动/删除），请重新选择图片。")
            return
        ...
        self.request_refresh()
        self._run_task_async(t)
        self.set_status(f"已重新执行：{sel}（会扣费）")

    def resume_remote_tasks(self, tasks: list[TaskItem]):
        """
        启动恢复向导用：批量继续 remote_id（不重新POST、不扣费）
        """
        if not tasks:
            return
        if not self.xintian_key:
            self.log_global("⚠️ 未加载 XINTIAN Key：无法自动继续 remote_id 任务，可先加载配置后手动继续。\n")
            return
        for t in tasks:
            rid = (t.remote_id or "").strip()
            if not rid:
            ...
            def make_runner(task_ref: TaskItem, remote_id: str):
                def runner():
                    def on_text(msg: str):
                        self.log_task(task_ref.task_id, msg)
                        p = parse_progress_percent(msg)
                        if p is not None:
                            task_ref.progress = p
                            self.mark_dirty()
                    ...
                    def on_done(video_url: str | None):
                        task_ref.video_url = video_url
                        if task_ref.stop_event and task_ref.stop_event.is_set():
                            task_ref.status = "Stopped"
                        else:
                            task_ref.status = "Success" if video_url else "Done(No URL)"
                        if task_ref.status in ("Success", "Done(No URL)"):
                            task_ref.progress = max(task_ref.progress, 100.0)
                        task_ref.last_check_at = now_str()
                        self.mark_dirty()
                        self.request_refresh()

                    def on_error(err: str):
                        task_ref.status = "Pending(Check)"
                        task_ref.last_error = str(err)[:500]
                        task_ref.last_check_at = now_str()
                        self.log_task(task_ref.task_id, f"\n⚠️ 自动继续轮询中断：{err}\n")
                        self.mark_dirty()
                        self.request_refresh()
                    ...
                    cfg = XintianConfig(
                        ...
                        prompt=task_ref.prompt,
                        image_path=task_ref.image_path,
                        poll_interval_sec=3,
                    )
                    run_xintian_poll_existing(cfg, remote_id, on_text, on_done, on_error, task_ref.stop_event or threading.Event())
                return runner

            if not self.executor:
                self.apply_concurrency()
            t.future = self.executor.submit(make_runner(t, rid))

        self.set_status(f"已批量继续 {len(tasks)} 个 Remote ID 任务（不扣费）")
        self.log_global(f"\n🔁 已批量继续 {len(tasks)} 个 Remote ID 任务（不扣费）\n\n")
    ...
    # 补查“失踪/无链接/超时中断”任务
    # =========================
    def refresh_pending_links(self, scope: str = "filtered", silent: bool = False):
        """
        scope:
          - filtered: 当前筛选列表范围
          - selected: 仅选中任务
        规则：
          - 只补查 XINTIAN（因为它有 status API 且我们保存了 remote_id）
          - status in {"Pending(Check)", "Done(No URL)", "Failed"} 且 remote_id 存在 → 允许补查
        """
        if not self.xintian_key:
            if not silent:
                messagebox.showinfo("提示", "未加载 XINTIAN Key（请先加载配置）。")
            return
        ...
        if not ids:
            if not silent:
                messagebox.showinfo("提示", "没有可补查的任务范围。")
            return
        ...
        for tid in ids:
            ...
            if not t:
                continue
            if (t.remote_id or "").strip() and (t.provider in ("xintian", "auto")):
                if t.status in ("Pending(Check)", "Done(No URL)", "Failed"):
                    targets.append(t)

        if not targets:
            if not silent:
                messagebox.showinfo("提示", "当前范围内没有可补查的任务（需 remote_id 且状态为 Pending/Done(No URL)/Failed）。")
            return
        ...
        def worker():
            ...
            for t in targets:
                ...
                status_url = build_xintian_video_status_url(XINTIAN_DEFAULT_BASE, rid)
                headers = {"Authorization": f"Bearer {self.xintian_key.strip()}"}
                timeout = (20, 60)

                try:
                    gr = requests.get(status_url, headers=headers, timeout=timeout, verify=True)
                    gr.encoding = "utf-8"
                    if gr.status_code != 200:
                        continue
                    obj = safe_json_loads(gr.text) or {}
                    ...
                    if url and isinstance(url, str):
                        ...
                        t.last_check_at = now_str()
                        if status and any(k in status.lower() for k in ("running", "processing", "queued")):
                            t.status = "Pending(Check)"
                        self.log_task(t.task_id, f"\n🔁 已补查：暂未返回链接（status={status or '-'}）\n")
                except Exception as e:
                    t.last_check_at = now_str()
                    self.log_task(t.task_id, f"\n🔁 补查失败（网络/超时）：{e}\n")
                    continue
            ...
            self.request_refresh()
            if not silent:
                self.root.after(0, lambda: messagebox.showinfo("补查完成", f"已补查：{checked} 条\n找回链接：{found} 条"))
    ...
    def open_download_dialog(self):
        ...
        mode_var = tk.StringVar(value="filtered")
        retry_var = tk.IntVar(value=2)
        skip_var = tk.BooleanVar(value=True)  # ✅ 默认跳过（保持现在行为）
        ...
        frm_skip = ttk.Frame(win)
        frm_skip.pack(fill="x", padx=14, pady=(4, 2))
        ttk.Checkbutton(frm_skip, text="跳过已下载/去重（URL/已存在/SHA重复）", variable=skip_var).pack(anchor="w")
        ...
        def start_download():
            ...
            th = threading.Thread(
                ...
                args=(tasks, download_root, int(retry_var.get()), bool(skip_var.get()), int(dl_workers_var.get()),
                      stop_flag,
    ...
    def _batch_download_worker(self, tasks: list, out_dir: Path, retries: int, skip_enabled: bool, dl_workers: int,
                               stop_flag: threading.Event,
                               overall_label, overall_bar, current_label, current_bar, cancel_btn):
        ...
        failed = 0
        done_count = 0
        log(f"\n📥 并发批量下载开始：{total} 个不同链接 | workers={dl_workers} | retries={retries} | skip={skip_enabled}\n")
        ...
        def prepare_target(url: str, t: TaskItem) -> tuple[Path, str]:
            ...
            # ✅ 关键：按 group 建目录归档
            group_name = safe_filename(t.group or "Ungrouped")
            group_dir = out_dir / group_name
            group_dir.mkdir(parents=True, exist_ok=True)

            return group_dir / filename, ext
        ...
        def job(url: str, t: TaskItem):
            ...
            with index_lock:
                if skip_enabled and url in by_url and by_url[url].get("file") and Path(by_url[url]["file"]).exists():
                    return ("skip_url", url, t.task_id, "", by_url[url]["file"])

            target, _ = prepare_target(url, t)

            if target.exists() and skip_enabled:
                with index_lock:
            ...
            # ✅ 不跳过时：避免覆盖同名文件（建议加后缀）
            if target.exists() and (not skip_enabled):
                # 在文件名后加 _dup1/_dup2...
                stem = target.stem
                suf = target.suffix
                k = 1
                while True:
                    cand = target.with_name(f"{stem}_dup{k}{suf}")
                    if not cand.exists():
                        target = cand
                        break
                    k += 1
            ...
            try:
                ...
                with index_lock:
                    # ✅ 只有在“允许跳过/去重”时，才按 sha 去重
                    if skip_enabled and sha in by_sha and by_sha[sha].get("file") and Path(
                            by_sha[sha]["file"]).exists():
                        try:
                            Path(final_path).unlink(missing_ok=True)
                        except Exception:
                            pass

                        existing = Path(by_sha[sha]["file"])

                        # ✅ 让当前 group 目录也有一份文件（复制）
                        try:
                            # target 必须是 prepare_target() 生成的当前 group 路径
                            if not Path(target).exists():
                                shutil.copy2(str(existing), str(target))

                            by_url[url] = {"file": str(target), "sha256": sha, "task_id": t.task_id}
                            self._save_download_index(index)
                            return ("skip_sha", url, t.task_id, sha, str(target))

                        except Exception:
                            # copy失败：退回复用 existing 路径
                            by_url[url] = {"file": str(existing), "sha256": sha, "task_id": t.task_id}
                            self._save_download_index(index)
                            return ("skip_sha", url, t.task_id, sha, str(existing))

                    # ✅ 不跳过：照常记录本次下载的新文件（允许同 sha 多份）
